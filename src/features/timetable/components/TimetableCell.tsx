...
function createStyleAttrFromLessons(lessons: TimetableItem[]) {
  const propertyObject = lessons.reduce((acc, lesson, index) => {
          ...
          >
              <>
                <div 
                  ...
                  data-time={`${lessonsTimes[innerLessons[0]?.number ?? 0 - 1]?.start}-${lessonsTimes[innerLessons[0]?.number ?? 0]?.end}`} data-number={innerLessons[0]?.number}
                />
